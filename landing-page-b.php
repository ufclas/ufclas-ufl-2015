<!DOCTYPE html>
<html lang="en" class="no-js">
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width,initial-scale=1">
	<title>Landing Page</title>
	<link href="css/style.css" rel="stylesheet">
	<link rel="apple-touch-icon" href="img/client-icon-iphone.png">
	<link rel="apple-touch-icon" sizes="72x72" href="img/client-icon-ipad.png">
	<link rel="apple-touch-icon" sizes="114x114" href="img/client-icon-iphone-retina.png">
	<link rel="apple-touch-icon" sizes="144x144" href="img/client-icon-ipad-retina.png">
	<link rel="icon" href="favicon.png" >
</head>

<body class="loading">
  <?php include 'inc/svg.php'; ?>
  <?php include 'inc/header.php'; ?>

  <div id="main" class="landing-page-hero-full">
  	<div class="hero-img gradient-bg" style="background-image:url(img/_temp1.jpg);">
	  	<h1>Landing page headline</h1>
	  </div>
	  <div class="hero-text">
	  	<div class="container">
	  		<div class="col-sm-8 col-sm-offset-2">
	  			<p>Lorem ised ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beata</p>
	  			<a href="#" class="btn">Learn More <span class="arw-right icon-svg"><svg><use xlink:href="/img/spritemap.svg#arw-right"></use></svg></span></a>
	  		</div>
	  	</div>
	  </div>
  </div>

	<div class="img-callout-wrapper hor-scroll-wrap" style="background-image:url(/img/bg-breaker.jpg)">
		<div class="container">
			<div class="row">
				<div class="col-sm-12 col-md-4 img-callout-wrap hor-scroll-el">
					<div class="img-callout">
						<img src="/img/_temp-square.jpg" alt="" class="img-full">
						<h2>Tertiary Headline</h2>
						<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Vitae, culpa, nisi! Maxime, commodi. Distinctio dolore perferendis asperiores corporis, dignissimos, nobis dolores! Laborum, quo expedita animi cum sed perferendis quae error.</p>
						<a href="#" class="read-more">Learn more</a>
					</div>
				</div>
				<div class="col-sm-12 col-md-4 img-callout-wrap hor-scroll-el">
					<div class="img-callout">
						<img src="/img/_temp-square.jpg" alt="" class="img-full">
						<h2>Tertiary Headline</h2>
						<p>Cum vero veritatis id rem mollitia, beatae odit quaerat expedita harum quidem nulla. Architecto labore incidunt aliquid laboriosam sit earum nesciunt, placeat dignissimos et, velit a. Optio est quas deleniti!</p>
						<a href="#" class="read-more">Learn more</a>
					</div>
				</div>
				<div class="col-sm-12 col-md-4 img-callout-wrap hor-scroll-el">
					<div class="img-callout">
						<img src="/img/_temp-square.jpg" alt="" class="img-full">
						<h2>Tertiary Headline</h2>
						<p>Qui iste eveniet vitae, ea culpa perspiciatis totam iusto esse! Animi vero dolor similique, natus provident placeat quidem cum. Non ducimus incidunt eveniet ipsum, facilis veritatis vero hic atque mollitia!</p>
						<a href="#" class="read-more">Learn more</a>
					</div>
				</div>
			</div>
		</div>
	</div>

	<div class="content-box-module">
		<div class="container">
			<div class="row">
				<div class="col-sm-5 content-box-img" style="background-image:url(img/_temp-landing-a-1.jpg)">
					<img src="http://dummyimage.com/470x532" alt="" class="visuallyhidden">
				</div>
				<div class="col-sm-7 content-box-copy">
					<h2>A Longer Secondary Headline Here Introducing This Section</h2>
					<p>Lorem ised ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae.</p>
					<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo.</p>
					<p>Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt. Neque porro quisquam est, qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit</p>
					<a href="#" class="read-more">learn more</a>
					<span class="category-tag orange">Section</span>
				</div>
			</div>
		</div>
	</div>

	<div class="breaker" style="background-image: url(/img/bg-breaker2.jpg);">
  	<div class="container">
  		<div class="row">
  			<div class="col-sm-8 col-sm-offset-2">
  				<h2>A Main Section Secondary Headline Here</h2>
  				<p>Lorem ised ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beata</p>
  				<a href="#" class="btn btn--white">Learn More <span class="arw-right icon-svg"><svg><use xlink:href="/img/spritemap.svg#arw-right"></use></svg></span></a>
  			</div>
  		</div>
  	</div>
  </div>


	<div class="content-box-module content-box-module--tweet">
		<div class="container">
			<div class="row">
				<div class="col-sm-6 col-sm-push-6 col-md-5 col-md-push-5 content-box-img" style="background-image:url(img/_temp-landing-a-1.jpg)">
					<div class="tweet-block">
						<div class="tweet-copy">
							<p>Dolor Sit Amet Consectetur Adipiscing Ligula Eget Dolor Ipsum Et Magis Dis Montes Com Dis Mvelit Sit Adipisci.</p>
							<div class="tweet-meta">
								<span class="btn-circle icon-svg icon-twitter"><svg><use xlink:href="/img/spritemap.svg#twitter"></use></svg></span>
								<a href="#" class="twitter-name">@TWITTERNAME</a>
							</div>
						</div>
						<a href="#" class="category-tag">Twitter</a>
					</div>
					<img src="img/_temp-landing-a-1.jpg" alt="" class="visible-mobile img-full">
				</div>

				<div class="col-sm-6 col-sm-pull-6 col-md-5 col-md-pull-5 content-box-copy">
					<h2>A Longer Secondary Headline Here Introducing This Section</h2>
					<p>Lorem ised ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae.</p>
					<p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo.</p>
				</div>
			</div>
		</div>
	</div>

	<?php include 'inc/footer.php'; ?>
	<?php include 'inc/scripts.php'; ?>
</body>
</html>